    ...
    <div className="space-y-6 mt-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Wasted %", value: `${Math.min(100, wastedPct)}%`, color: "text-iv-yellow" },
        ].map((item) => (
        ...
        {wastedMinutes > 0 && (
          <div className="mt-4 bg-red-500/10 border border-red-500/30 rounded-xl p-4 flex items-center gap-3">
            ...
            <p className="text-sm text-iv-text">
              ...
              <span className="text-iv-text-muted ml-2">(≈{Math.min(100, wastedPct)}% of total charging time)</span>
            </p>
          </div>
        )}
      </div>
